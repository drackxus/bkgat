...
function evento_register(){
	...
	$pregunta1  = isset($_POST['pregunta1']) ? $_POST['pregunta1'] : false;
	$pregunta1 = sanitize_text_field($_POST['pregunta1']);

	$pregunta2  = isset($_POST['pregunta2']) ? $_POST['pregunta2'] : false;
	$pregunta2 = sanitize_text_field($_POST['pregunta2']);

	$pregunta3  = isset($_POST['pregunta3']) ? $_POST['pregunta3'] : false;
	$pregunta3 = sanitize_text_field($_POST['pregunta3']);

	$inscripciones = get_post_meta($post, 'inscripciones', true);

	array_push($inscripciones, (object)[
		'user' => $user,
		'post' => $post,
		'pregunta1' => $pregunta1,
		'pregunta2' => $pregunta2,
	...
	if (!$user || !$post) {
		...
		//Si todo ha ido bien, agregamos los campos adicionales
		//if (!is_wp_error($user)) {
		update_post_meta($post, 'inscripciones', $inscripciones);
		$lista_inscripciones = get_post_meta($post, 'inscripciones', true);
		// echo $lista_inscripciones;
		echo "Te haz inscrito correctamente";
		wp_die();
		//}
	}	
}
